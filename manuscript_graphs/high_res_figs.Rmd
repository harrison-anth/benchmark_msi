...
sensor <- function(prefix,tool,tcga,raw,fresh){
  ...
  if(raw==FALSE){
    temp21 <- filter(results, status != "NA") 
    temp21 <- filter(temp21, status != "?")
    temp21 <- filter(temp21, tot_sites > 4)
...
#grid arrange function for shared common legend
...
all_confusion <- rbind(confuse(df=rbind(wxs_pro,wgs_pro,rna_pro,mgiseq_pro,korea_pro,candiolo_pro,helicase_pro),prefix = 'All',threshold=10,tool='MSIsensor-pro'),
                         confuse(df=rbind(wxs_sensor2,wgs_sensor2,rna_sensor2,mgiseq_sensor2,korea_sensor2,candiolo_sensor2,helicase_sensor2),
          prefix = 'All',threshold=20,tool='MSIsensor2'),
  confuse(df=rbind(wxs_msings,wgs_msings,rna_msings,mgiseq_msings,korea_msings,candiolo_msings,rna_msings,helicase_msings),
          prefix = 'All',threshold=.2,tool='mSINGS'),
...
all_confuzed <- rbind(TCGA_all_confusion,
                      extra_confusion,
                      all_confusion)
all_confuzed$"F1 score" <- all_confuzed$F1

#all_confuzed$Dataset <- c(rep('Dataset8',6),rep('Dataset7',6),rep('Dataset6',5),rep('Dataset2',4),rep('Dataset1',6),rep('Dataset4',4),rep('Dataset3',6),rep('Dataset5',5),rep('Dataset9',8))
...
all_confuzed %>% pivot_wider(id_cols = Dataset,names_from='Tool',values_from = (c('Precision','Recall','Specificity','Accuracy','F1 score'))) -> temp
...
#to save plot 
tiff("F:/temp/test.tiff",units='px',width = 6500,height = 5500,res = 500)
hm <- Heatmap(t(trixy),na_col='darkkhaki',row_split = c(rep(unique(factor((all_confuzed$Tool))),5)),
...
cbPalette <- c("#ff0101","goldenrod", "#7061cf",  "yellow2","#bebebe" )
...
x <- evalm(models,
           positive='High',
                gnames=names(models),
                cols = cbPalette)
wxs_roc <- x$roc +ggtitle('A')+xlab('Recall')+theme(legend.position = 'none')
wxs_prc <- x$proc +ggtitle('B')+xlab('Recall')+theme(legend.position = 'none')

                
#ggsave('F:/temp/roc_wxs_hd.tiff',x$roc,units='px',dpi=500,height=5000,width = 5000)
#ggsave('F:/temp/prc_wxs_hd.tiff',x$proc,units='px',dpi=500,height = 5000,width = 5000)
...
x <- evalm(models,
           positive='High',
                gnames=names(models),cols = cbPalette)
wgs_roc <- x$roc +ggtitle('C')+xlab('Recall')+theme(legend.position = 'none')
wgs_prc <- x$proc +ggtitle('D') +xlab('Recall')+theme(legend.position = 'none')



roc_list <- list(wxs_roc,wxs_prc,wgs_roc,wgs_prc)
# function to extract legend from plot 
    
#ggsave('F:/temp/roc_wgs_hd.tiff',x$roc,units='px',dpi=500,height=5000,width = 5000)
#ggsave('F:/temp/prc_wgs_hd.tiff',x$proc,units='px',dpi=500,height=5000,width=5000)

all_rocs <- arrangeGrob(grobs=roc_list,ncol=2)

ggsave(filename = 'F:/temp/all_rocs_hd.tiff',plot = all_rocs,device = 'tiff',dpi = 500,width = 5000,height = 4500,units = 'px')
#note to add a common legend for parsimony was only able to be done with photoshop after generating this figure. 
#The use does not impact the results in anyway, but it will be worthwhile to find a way to create a common legend
#that also includes all the AUC values for all four plots. 
...
x <- evalm(models,
           positive='High',
                gnames=names(models),cols = c("goldenrod","yellow","#bebebe"))
##ggsave('F:/temp/roc_rna_hd.tiff',x$roc,units='px',dpi=500,height = 5000,width = 5000)
#ggsave('F:/temp/prc_rna_hd.tiff',x$proc,units='px',dpi=500,height=5000,width=5000)


rna_roc <- x$roc +ggtitle('A')+xlab('Recall')
rna_prc <- x$proc +ggtitle('B') +xlab('Recall')


roc_list <- list(rna_roc,rna_prc)
# function to extract legend from plot 
    
#ggsave('F:/temp/roc_wgs_hd.tiff',x$roc,units='px',dpi=500,height=5000,width = 5000)
#ggsave('F:/temp/prc_wgs_hd.tiff',x$proc,units='px',dpi=500,height=5000,width=5000)

printme <- arrangeGrob(grobs=roc_list,ncol=1,nrow=2)
ggsave(filename = 'F:/temp/supplementary_figure1_hd_take2.tiff',plot = printme,device = 'tiff',units = 'px',dpi = 300,height=2500,width=2500)

```

```{r upset-plot,eval=FALSE}

all_df <- rbind(genekor_pro,genekor_msings,genekor_sensor2,candiolo_pro,candiolo_msings,candiolo_sensor2,helicase_pro,helicase_msings,helicase_sensor2,korea_sensor2,korea_pro,korea_msings,korea_sensor, korea_mantis,tso_fin)
...
colnames(tso5002) <- c('samp_id','MSIsensor-pro','MSIsensor2','mSINGS','Illumina')
tso5003 <- data.frame(samp_id=tso5002$samp_id,Illumina=tso5002$Illumina,'MSIsensor-pro'=tso5002$`MSIsensor-pro`,mSINGS=tso5002$mSINGS,MSIsensor2=tso5002$MSIsensor2)



upset22 <- upset(tso5003,mainbar.y.label = "TSO500 Panel Intersctions", sets.x.label = "MSI-H cases per tool",text.scale =   c(3, 2, 1.8, 2, 2, 2.8))
...
colnames(genekor2) <- c('samp_id','MSIsensor-pro','MSIsensor2', 'mSINGS','MSIcall')



upset11 <- upset(genekor2,mainbar.y.label = "161 Marker Panel Intersctions", sets.x.label = "MSI-H cases per tool",text.scale = c(3, 2, 1.8, 2, 2, 2.8))

#upset1 <- as.ggplot(upset11)
#upset1 <- upset1+ggtitle('A')+
#  theme(plot.title = element_text(hjust = 0.1, vjust = -10,size=20))
#upset2 <- as.ggplot(upset22)
#upset2 <- upset2 + ggtitle('B')+
#  theme(plot.title = element_text(hjust = 0.1, vjust = -10,size=20))

#upset_plots <- list(upset1,upset2)

#upsets <- arrangeGrob(grobs=upset_plots,ncol=1,nrow=2)


uu1 <- cowplot::plot_grid(NULL, upset11$Main_bar, upset11$Sizes, upset11$Matrix,
                            nrow=2, align='hv', rel_heights = c(3,1),
                           rel_widths = c(2,3),labels='A')
uu2 <- cowplot::plot_grid(NULL, upset22$Main_bar, upset22$Sizes, upset22$Matrix,
                            nrow=2, align='hv', rel_heights = c(3,1),
                           rel_widths = c(2,3),labels = 'B')
upsets <- arrangeGrob(grobs=list(uu1,uu2),ncol=1,nrow=2)

ggsave('F:/temp/figure4_hd.tiff',plot = upsets,device = 'tiff',width = 5800,height = 6800,units = 'px',dpi = 500)
...
for(i in 1:nrow(wgs_presrd)){
temp <- wgs_presrd[i,]
if(temp$Tool == "MSIsensor-pro" |
   temp$Tool == "MSIsensor" | 
   temp$Tool == "mSINGS" |
   temp$Tool == "MANTIS" | 
   temp$Tool == "MSIsensor2"){
...
wgs_presrd %>%
 ...
 ggtitle('Figure 3',subtitle = 'TCGA Boxplots of MSI Scores')+ ylab('MSI-score')+xlab('Sequencing type')+
  ...
  wgs_presrd %>%
      ggpaired(x='seq',y='value', id = "case_id",
         color = "seq", line.color = "gray90", line.size = .2,
         palette = "jco")+
  stat_compare_means(paired = TRUE,vjust=-1)+  facet_wrap(~Tool,scales="free_y",nrow=1)+theme_classic()+
  ylab('MSI score')+xlab('Sequencing type')+ theme(legend.position = "none")+
    theme(strip.text.x = element_text(size = 12)) ->preseder
  
  ggsave(filename = 'F:/temp/figure3_hd.tiff',plot = preseder,device = 'tiff',dpi = 500,width = 5000,height = 4500,units = 'px')
...
fig5 <- ggplot(final_df,aes(x=threshold,y=F1,color=Tool))+
  geom_line()+facet_wrap(~Dataset
                         )+theme_pubr()+
 #('Figure 5',subtitle = 'Performance on additional datasets by threhsold')+
  xlab('Threshold')+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust=.5)) +ylab("F1 Score")+scale_color_manual(values = cbPalette)

ggsave(filename = 'F:/temp/figure5_hd.tiff',plot = fig5,device = 'tiff',dpi = 500,width = 5000,height = 4500,units = 'px')


sup_fig5<- ggplot(final_df,aes(x=threshold,y=Precision,color=Tool))+
  geom_line()+facet_wrap(~Dataset
                         )+theme_pubr()+
 #('Figure 5',subtitle = 'Performance on additional datasets by threhsold')+
  xlab('Threshold')+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust=.5))+scale_color_manual(values = cbPalette)
ggsave(filename = 'F:/temp/supplementary_figure5_hd.tiff',plot = sup_fig5,device = 'tiff',dpi = 300,width = 2500,height = 2000,units = 'px')



sup_fig3<- ggplot(final_df,aes(x=threshold,y=Specificity,color=Tool))+
  geom_line()+facet_wrap(~Dataset
                         )+theme_pubr()+
 #('Figure 5',subtitle = 'Performance on additional datasets by threhsold')+
  xlab('Threshold')+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust=.5))+scale_color_manual(values = cbPalette)
ggsave(filename = 'F:/temp/supplementary_figure3_hd.tiff',plot = sup_fig3,device = 'tiff',dpi = 300,width = 2500,height = 2000,units = 'px')




sup_fig2 <- ggplot(final_df,aes(x=threshold,y=Accuracy,color=Tool))+
  geom_line()+facet_wrap(~Dataset
                         )+theme_pubr()+
 #('Figure 5',subtitle = 'Performance on additional datasets by threhsold')+
  xlab('Threshold')+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust=.5))+scale_color_manual(values = cbPalette)
ggsave(filename = 'F:/temp/supplementary_figure2_hd.tiff',plot = sup_fig5,device = 'tiff',dpi = 300,width = 2500,height = 2000,units = 'px')




sup_fig4<- ggplot(final_df,aes(x=threshold,y=Recall,color=Tool))+
  geom_line()+facet_wrap(~Dataset
                         )+theme_pubr()+
 #('Figure 5',subtitle = 'Performance on additional datasets by threhsold')+
  xlab('Threshold')+
  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust=.5))+scale_color_manual(values = cbPalette)
ggsave(filename = 'F:/temp/supplementary_figure4_hd.tiff',plot = sup_fig4,device = 'tiff',dpi = 300,width = 2500,height = 2000,units = 'px')
