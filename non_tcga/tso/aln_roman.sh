...
start=$(( $end - 1 ))
one=fastq/$(sed "${start}q;d" romans_access_list.txt).fastq.gz
two=fastq/$(sed "${end}q;d" romans_access_list.txt).fastq.gz

header=$(zcat $one | head -n 1)
id=$(echo $header | head -n 1 | cut -f 1-4 -d":" | sed 's/@//' | sed 's/:/_/g')
sm=$(echo $header | head -n 1 | grep -Eo "[ATGCN]+$")

gnorts=$(sed "${start}q;d" romans_access_list.txt)
